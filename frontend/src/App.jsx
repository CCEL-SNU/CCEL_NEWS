...
function NewsCard({d,bm,onBm}){
  const color=cc(d.category||d.cat);
  const catLabel=CATEGORIES.find(c=>c.id===(d.category||d.cat))?.label||(d.category||d.cat||"");
  ...
  return <article className="ccel-card ccel-fade" style={{
    ...
    <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",marginBottom:8}}>
      {catLabel&&<Badge label={catLabel} color={color}/>}
...
function TrendChart({papers}){
  // Build trend data from actual papers
  const byMonth={};
  const byCat={};
  (papers||[]).forEach(p=>{
    const d=p.date||"";
...
export default function App(){
  ...
  useEffect(()=>{setPage(1);},[cat,grps,q,bmOnly,sort,view]);
  ...
  const filtered=useMemo(()=>{
    return papers.filter(d=>{
      const dcat=d.category||d.cat||"";
      if(cat!=="all"&&dcat!==cat)return false;
  ...
  },[papers,cat,grps,bmOnly,bms,q,sort]);
  ...
  const todayCnt=papers.filter(p=>p.date===genDate).length;
    ...
    <div style={{maxWidth:980,margin:"0 auto",padding:"20px 20px 0"}}>
      ...
      <div style={{background:C.bgAlt,border:`1px solid ${C.borderLight}`,padding:"14px 16px",marginBottom:22}}>
        ...
        <div>
          ...
          <div style={{display:"flex",gap:5,flexWrap:"wrap",marginTop:5}}>
            {CATEGORIES.map(c=><Pill key={c.id} label={c.label} active={cat===c.id} color={cc(c.id)} onClick={()=>setCat(c.id)}/>)}
          </div>
        </div>
      </div>
      ...
      {view==="feed"&&<div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {paged.length===0?<div style={{textAlign:"center",padding:50,color:C.gray500,fontSize:13}}>No results.</div>
      ...
      {/* MILESTONES (placeholder - will be populated later) */}
      {view==="milestones"&&<div>
        <div style={{textAlign:"center",marginBottom:20}}>
