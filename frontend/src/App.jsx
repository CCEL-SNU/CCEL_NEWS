...
function normalizeNotableTitle(t){
  return String(t||"").toLowerCase().replace(/\s+/g," ").trim();
}

/** Match digest `notable_papers[].title` to a row in Paper feed (titles may be truncated in JSON). */
function findPaperForNotable(papers, notableTitle){
  if(!papers?.length||!notableTitle)return null;
  const n=normalizeNotableTitle(notableTitle);
  if(n.length<6)return null;
  for(const p of papers){
    const pt=normalizeNotableTitle(p.title||"");
    if(pt&&pt===n)return p;
  }
  for(const p of papers){
    const pt=normalizeNotableTitle(p.title||"");
    if(!pt)continue;
    if(pt.includes(n)||n.includes(pt))return p;
  }
  const prefixLen=Math.min(56,n.length);
  if(prefixLen>=20){
    const pre=n.slice(0,prefixLen);
    for(const p of papers){
      const pt=normalizeNotableTitle(p.title||"");
      if(pt.startsWith(pre))return p;
    }
  }
  return null;
}
...
function NewsCard({d,bm,onBm,groups}){
  ...
  return <article className="ccel-card ccel-fade" style={{
    ...
    {d.relevance_reason&&<p style={{fontSize:12.5,color:C.gray600,margin:"10px 0 0",lineHeight:1.68,borderLeft:`3px solid ${C.borderLight}`,paddingLeft:12}}><span style={{fontWeight:700,color:C.gray500,fontSize:11}}>선정 이유 </span>{d.relevance_reason}</p>}
...
function CategoryTrends({trends,papers,bms,onBm,groups}){
  ...
  return <div className="ccel-card" style={{background:C.white,border:`1px solid ${C.borderLight}`,padding:0,boxShadow:C.shadow,overflow:"hidden"}}>
    ...
    <div style={{padding:"0"}}>
      ...
      <StructuredDigestContent digest={digest} accentColor={color} papers={papers} bms={bms} onBm={onBm} groups={groups}/>
    </div>
  </div>;
}

function StructuredDigestContent({digest,accentColor,papers,bms,onBm,groups}){
        ...
        {notablePapers.map((np,i)=>{
          ...
          return <div key={`np-${i}-${np.title?.slice(0,24)||i}`} style={{marginBottom:22}}>
            ...
            {matched&&onBm
              ...
              :<div style={{
                padding:"14px 16px",border:`1px solid ${C.borderLight}`,borderRadius:4,background:C.bgAlt,
                fontSize:12,color:C.gray600,lineHeight:1.5,
              }}>
...
function weeklyDigestPaperCount(d){
  if(!d||typeof d!=="object")return null;
  const n=d.weekly_digest_paper_count;
  return typeof n==="number"?n:null;
}

function WeeklyDigest({digest,digestInGroup,digestOutGroup,papers,bms,onBm,groups}){
  const [digestTab,setDigestTab]=useState("all");
  if(!digest)return null;
  ...
  const active=digestTab==="all"?digest:digestTab==="in"?digestInGroup:digestOutGroup;
  const tabLabel=digestTab==="all"?"전체":digestTab==="in"?"그룹 소속 논문":"그룹 외 논문";
  const pc=weeklyDigestPaperCount(active);
  const metaDate=(active&&active.generated_at)?active.generated_at:digest.generated_at;
  ...
  return <div className="ccel-card" style={{background:C.white,border:`1px solid ${C.borderLight}`,padding:0,boxShadow:C.shadow,overflow:"hidden"}}>
    <div style={{padding:"16px 20px",borderBottom:`1px solid ${C.borderLight}`}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
        ...
        <div style={{flex:1}}>
          <h4 style={{fontSize:15,fontWeight:700,margin:0,color:C.textDark}}>Weekly Digest</h4>
          <p style={{fontSize:11,color:C.gray500,margin:0}}>{tabLabel} · Auto-generated by Gemini{metaDate?` · ${metaDate.slice(0,10)}`:""}</p>
        </div>
      </div>
      <div style={{display:"flex",background:C.bg,borderRadius:6,padding:2,gap:0}}>
        {[
          ...
          {id:"in",label:"그룹 소속"},
          {id:"out",label:"그룹 외"},
        ].map(v=><button key={v.id} type="button" onClick={()=>setDigestTab(v.id)} style={{
          padding:"5px 12px",border:"none",borderRadius:5,cursor:"pointer",fontFamily:"inherit",
          fontSize:11,fontWeight:v.id===digestTab?700:400,
          background:v.id===digestTab?C.white:"transparent",
          color:v.id===digestTab?C.textDark:C.gray500,
          boxShadow:v.id===digestTab?"0 1px 3px rgba(0,0,0,.1)":"none",
          transition:"all .15s",
        }}>{v.label}</button>)}
      </div>
    </div>

    {digestTab!=="all"&&active==null&&<div style={{padding:"20px",textAlign:"center",color:C.gray500,fontSize:13}}>
      이 범위의 digest가 아직 없습니다. 최신 파이프라인에서 생성되면 표시됩니다.
    </div>}

    {digestTab!=="all"&&active!=null&&pc===0&&<div style={{padding:"20px",textAlign:"center",color:C.gray500,fontSize:13}}>
      이번 주 기간({active?.weekly_digest_window_days!=null?`${active.weekly_digest_window_days}일`:"7일"}) 안에 이 범위에 해당하는 논문이 없습니다.
    </div>}

    {(()=>{
      if(digestTab!=="all"&&active==null)return null;
      if(digestTab!=="all"&&active!=null&&pc===0)return null;
      const d=digestTab==="all"?digest:active;
      const sections=d?.sections||[];
      const hotIssues=d?.hot_issues||[];
      if(sections.length===0&&hotIssues.length===0)return <div style={{padding:"20px",textAlign:"center",color:C.gray500,fontSize:13}}>표시할 내용이 없습니다.</div>;
      return <StructuredDigestContent digest={d} accentColor={C.accent} papers={papers} bms={bms} onBm={onBm} groups={groups}/>;
    })()}
  </div>;
}
...
export default function App(){
  ...
  const digestInGroup=data?.weekly_digest_in_group??null;
  const digestOutGroup=data?.weekly_digest_out_group??null;
    ...
    <div style={{maxWidth:980,margin:"0 auto",padding:"20px 20px 0"}}>
      ...
      {view==="landscape"&&<div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div id="landscape-weekly-digest" className="ccel-landscape-section"><WeeklyDigest digest={digest} digestInGroup={digestInGroup} digestOutGroup={digestOutGroup} papers={papers} bms={bms} onBm={toggleBm} groups={allGroups}/></div>
          <div id="landscape-group-digests" className="ccel-landscape-section"><GroupDigests groupDigests={groupDigests} papers={papers} bms={bms} onBm={toggleBm} groups={allGroups}/></div>
          <div id="landscape-category-digests" className="ccel-landscape-section"><CategoryTrends trends={categoryTrends} papers={papers} bms={bms} onBm={toggleBm} groups={allGroups}/></div>
