...
def generate_weekly_digest(papers: List, config: dict = None):
    ...
    logger.info(
        "Weekly digest: %s papers in last %s days (by date), %s rows before dedupe/window",
        len(papers),
        window_days,
        raw_n,
    )
...
- Be specific: mention materials, methods, and quantitative results where available."""
    ...
    prompt = f"""아래 목록은 각 논문의 `date` 필드(YYYY-MM-DD, 발행·게재일)가 오늘 기준 지난 {window_days}일 이내인 항목만 포함합니다. 이 목록만 근거로 작성하세요. 목록에 없는 논문은 언급하지 마세요.
...
notable_papers: 실제 novelty가 있는 논문만 2-3편. 단순 성능 개선은 제외.{suffix}"""
...
hot_issues: 1-2개만. notable_papers: novelty가 있는 논문만 최대 2편. 없으면 빈 배열.{suffix}"""
