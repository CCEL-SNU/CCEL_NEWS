...
def _format_paper_list(papers: List, max_count: int = 50) -> str:
...
def _generate_structured_digest(
    ...
    paper_list = _format_paper_list(papers, 80 if depth == "full" else 30)
    ...
    if depth == "full":
        prompt = f"""Analyze the following papers from {context_intro} and produce a structured research digest.
...
{{
  ...
  "notable_papers": [
    {{
      "title": "논문 제목 (원문 그대로)",
      "reason": "왜 주목할 만한지 1문장 (비판적 novelty 평가)"
    }}
  ]
}}
...
notable_papers: 실제 novelty가 있는 논문만 2-3편. 단순 성능 개선은 제외."""
    else:
        prompt = f"""Analyze the following papers from {context_intro} and produce a brief research digest.
...
{{
  ...
  "notable_papers": [
    {{
      "title": "논문 제목 (원문 그대로)",
      "reason": "왜 주목할 만한지 1문장"
    }}
  ]
}}

hot_issues: 1-2개만. notable_papers: novelty가 있는 논문만 최대 2편. 없으면 빈 배열."""

    response = _call_gemini(prompt, _DIGEST_SYSTEM, config)
    ...
    return {
        "depth": depth,
        "hot_issues": [],
        "sections": [{"title": "Digest", "content": (response or "Generation failed.")[:1000]}],
        "notable_papers": [],
        "generated_at": datetime.now().isoformat(),
    }
